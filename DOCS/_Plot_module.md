...
The plot module includes a series of plots to visualize and characterize grain size populations. All methods of the *plot* module can be invoked by writing ```gst.plot.*```, where * refers to the plot to be used.

> 👉 If you write ``gst.plot.`` and then hit the tab key a menu will pop up with all the methods implemented in the module

The main method is ```gst.plot.distribution()```. The method allows to visualize the grain size population using the histogram and/or the kernel density estimate (KDE) and provides the location of the different averages (Fig 1). The simplest example of use would be to pass the column with the diameters as follows:
...
gst.plot.distribution(dataset['diameters'])
```
...
*Figure 1. The ```gst.plot.distribution()``` plot with default options. This shows the histogram and the kernel density estimate (KDE) of the distribution, and the location of the averages estimated by the function ``gst.summarize`` by default*

The method returns a plot, the number of classes and bin size of the histogram, and the bandwidth (or kernel) of the KDE. The ```gst.plot.distribution()``` method contains different options that we will commented on in turn:
...
```python
gst.plot.distribution(dataset['diameters'], plot='kde')
```
...
```python
gst.plot.distribution(dataset['diameters'], plot='hist', binsize='doane')
```
...
```python
gst.plot.distribution(dataset['diameters'], plot='hist', binsize=10.5)
```
...
```python
gst.plot.distribution(dataset['diameters'], plot='kde', bandwidth=5.0)
```

Note, however, that the bandwidth affects the location of the KDE-based mode. For consistency, you should use the same method or bandwidth used when calling the ```gst.summarize``` method.
...
```python
gst.plot.qq_plot(dataset['diameters'], figsize=(6, 5))
```
...
```python
gst.plot.area_weighted(dataset['diameters'], dataset['Area'])
```
...
```python
gst.plot.normalized(dataset['diameters'], avg='amean')
```
...
```python
gst.plot.normalized(dataset['diameters'], avg='median', bandwidth=0.1, figsize=(6, 5))
```
