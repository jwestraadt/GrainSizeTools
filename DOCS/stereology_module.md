...
text_file : string or None, optional
    if the user specifies a name, the function will store a csv or txt file
...
The only input required is the distribution of apparent diameters (``diameters``). From here the user can specify the number of classes (``numbins``), calculate the volume occupied by a grain size fraction (``calc_vol``), generate a tabular (csv or txt) file with the data (``text_file``), retrieve some data for further calculations (``return_data``) or set the initial boundary of the histogram (``left_edge``) if needed. We will see some examples below.
...
gst.stereology.Saltykov(dataset['diameters'], numbins=11, calc_vol=50)
```
...
If we want to save the data generated by Saltykov's method to a text file, we need to specify the file name and type, csv or txt, in the ``text_file`` parameter as follows.

```python
gst.stereology.Saltykov(dataset['diameters'], numbins=11, text_file='mydata.csv')
```
...
```python
mid_points, densities = gst.stereology.Saltykov(dataset['diameters'], numbins=11, return_data=True)
...
So if you want to estimate the volume fraction occupied by each grain size class, you could do this as follows
...
To apply this method, we use the ``two_step()`` function of the stereology module. This function has the following parameters:

```reStructuredText
diameters : array_like
    the apparent diameters of the grains

class_range : tuple or list with two values, optional
...
The only data required is the distribution of apparent diameters (``diameters``). From here, the user can only specify a range of number of classes (``class_range``), if necessary, an algorithm will take care of estimating the optimal number of classes within this range.

```python
gst.stereology.two_step(dataset['diameters'])
```
...
Briefly, we determined the probability density function (PDF) of the lognormal distribution and then estimated the volume fraction occupied by a given range of grain size values (here 75 and 25) by integrating the PDF over that range. This is equivalent to finding the cumulative distribution function (CDF) values at 75 and 25 and subtracting them. If the grain size population does indeed resemble a lognormal distribution, this method of calculating volume fractions is superior to the Saltykov method.
